...
DIRECTORIES = src src/board src/utility

# Include paths for project and Boost headers
INC = $(addprefix -I$(SELF_DIR)/,$(DIRECTORIES)) -I$(SELF_DIR)/include -I/usr/local/include

# List of source files (all .cpp files)
SOURCES = $(wildcard $(addsuffix /*.cpp,$(DIRECTORIES))) $(wildcard *.cpp)

# Debug flags (to use for development)
CXXFLAGS = -std=c++23 -DDEBUG -fsanitize=address -I/usr/include -Wall -g -MMD -O2 -Werror=vla -Werror=vla ${INC}

# Linking flags (add Boost libraries here)
LDFLAGS = -fsanitize=address \
          -lboost-filesystem -lboost-json -lboost-system \
